...
import { ClaudeOAuthLogin, parseManualCode } from "./oauth-login.js";
...
describe("Claude OAuth login", () => {
  ...
  it("rejects a mismatched state before token exchange", async () => {
    ...
    const login = new ClaudeOAuthLogin({ tokenUrl: serverUrl });
    const started = login.start();
    await expect(
      login.complete({
  ...
  it("expires an in-memory login session after ten minutes", async () => {
    let now = 1_000;
    const login = new ClaudeOAuthLogin({ now: () => now });
    const started = login.start();
    now += 10 * 60 * 1_000;
    await expect(
  ...
  it("can cancel a pending login session", async () => {
    const login = new ClaudeOAuthLogin({ now: () => 0 });
    const started = login.start();
    expect(login.cancel(started.sessionId)).toBe(true);
    expect(login.cancel(started.sessionId)).toBe(false);
    await expect(
      login.complete({ sessionId: started.sessionId, pasted: "code" }),
