...
import { CodexDeviceLogin, codexJwtExpiryMs } from "./device-login.js";
...
describe("Codex device login", () => {
  it("polls pending then exchanges and returns the ID-token account claims", async () => {
  ...
  it("expires the session after ten minutes without polling auth", async () => {
    ...
    const login = new CodexDeviceLogin({ authBaseUrl, now: () => now });
    const started = await login.start();
    now += 10 * 60 * 1_000;
    expect(await login.poll({ sessionId: started.sessionId })).toEqual({
      status: "error",
...
describe("codexJwtExpiryMs", () => {
  it("converts exp seconds to milliseconds", () => {
    expect(codexJwtExpiryMs(jwt({ exp: 1000 }))).toBe(1_000_000);
    expect(codexJwtExpiryMs("malformed")).toBeNull();
  });
});
